            ...
            <div class="content_box">
                <div class="content_msg">
                    <div class="mouldbox_tit">
                        <div style="font-weight: 600;min-width: 70px">文章名：</div><div>{{tableData.article_name}}</div>
                    </div>
                    <div class="mouldbox_tit" >
                        <div style="font-weight: 600;min-width: 70px">作者：</div><div>{{tableData.article_author}}</div>
                    </div>
                    <div class="mouldbox_tit">
                        <div style="font-weight: 600;min-width: 70px">索引信息：</div><div>{{tableData.article_message}}</div>
                    </div>
                    <div class="sheji">
                        <div style="font-weight: 600">涉及变量：</div>
                        ...
                        <!--</div>-->
                        <div>
                            <a :href="baseURL+'#/terrace_fun_item_detail?id='+tableData.var[index].id+'&&type=3&&ids='+$route.query.id" v-for="(blname,index) in tableData.var" style="color:#009E79;cursor: pointer;display: block" target="_Blank">
                            <span>
                                    {{blname.name}}（<span v-for="(blname_item,index) in blname.dis">{{blname_item.name}}<span v-if="index!=blname.dis.length-1">，</span></span>）<span v-if="index!=tableData.var.length-1">，</span>
                                </span>
                            </a>
                    ...
                    <div class="mouldbox_bot_box">
                        <div class="mouldbox_left">
                            <el-row style="padding: 0;display: flex;font-size: 14px">
                                <div style="font-weight: 600;min-width: 70px">概述：</div><div>{{tableData.article_content}}</div>
                            </el-row>
                        </div>
